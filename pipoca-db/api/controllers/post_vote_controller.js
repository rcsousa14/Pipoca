...
exports.store = async({body, decoded}, res, ) => {
    try {
        ...
        //todo see if there is a patch
        if(vote){
            const updated_vote = await models.post_vote.update({id: vote.id, user_id: decoded.id, is_voted, post_id}, {
                
                 where: { id: vote.id },
            });
            //find status for update
            return res.status(200).send({message: "updated", updated_vote });
        }
        const add_vote = await models.post_vote.create({user_id: decoded.id, post_id, is_voted});
